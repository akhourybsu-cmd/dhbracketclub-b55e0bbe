...
export function uncertainInjury(injuries: Injury[]) {
  return injuries.some(injury => /\b(out|doubtful|questionable|reserve|suspended|suspension|pup|ir|nfi)\b/i.test(
    `${injury.status || ''} ${injury.type?.name || ''} ${injury.type?.description || ''} ${injury.type?.abbreviation || ''}`,
  ));
}

export function verifyPlayerAvailability(roster: EspnRoster, depth: EspnDepthChart, playerId: string, teamId: string, year: number) {
  if (roster.season?.year !== year || depth.season?.year !== year
      || String(roster.team?.id) !== teamId || String(depth.team?.id) !== teamId) return 'Roster could not be verified for this team and season.';
  const player = (roster.athletes || []).flatMap(group => group.items || []).find(item => String(item.id) === playerId);
  if (!player || player.status?.type !== 'active') return 'Player is not on this team’s active roster.';
...
export function validateBoardSlate(
  ...
  remainingOnly = false, lockMinutes = 10,
) {
  ...
  for (const game of games) {
    ...
    if (game.status !== 'scheduled' || competition?.status?.type?.state !== 'pre'
        || competition.status.type.completed || !(Date.parse(game.kickoff_at) - lockMinutes * 60_000 > now)) {
      if (remainingOnly) continue;
      throw new Error('This week has already started. Load the next unlocked week.');
    }
    eligible.push(game);
  }
  if (!eligible.length) throw new Error('No unstarted games remain before their lock cutoff.');
  return eligible;
}
