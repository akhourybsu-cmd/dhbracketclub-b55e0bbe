-- COPY/PASTE THIS ENTIRE FILE. Requires the original Crazy Chain tables only;
-- it also includes the necessary setup if the catch-up migration was not run.
-- Pick'em deadlines and existing saved picks/target snapshots are not changed.
begin;
