export function chainAvailabilityNote(market: {
  ...
  if (market.availability_status !== 'verified') return market.availability_note || 'Awaiting player availability recheck.';
  const checked = Date.parse(market.availability_checked_at || '');
  if (!Number.isFinite(checked) || now - checked > 24 * 60 * 60_000 || checked > now + 60_000) return 'Availability check expired; new selections paused.';
  return null;
}
